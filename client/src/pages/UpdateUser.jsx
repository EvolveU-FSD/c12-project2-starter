...
function UpdateUser() {
    ...
    return (
        <div className={`app-header ${dark ? "dark" : ""}`}>
            ...
            <div>Updated team:
                <input type="text" value={updatedTeamName} onChange={e=>setUpdatedTeamName(e.target.value)}/>
            </div>
            <div>
                <button onClick={()=>updateUser({name:updatedUserName, team: updatedTeamName})}>Update User</button>
            </div>
        </div>
    )
}
